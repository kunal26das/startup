 ...
 *
 * Every task in a wave depends only on earlier waves. Tasks may run at the same time when
 * their thread requirements allow it; a worker task must not call into [AppInitializer].
 * [Startup.install] hands each wave over in turn and waits for [run] to return before
 * executing the next one, so the ordering,
 ...
 * of a task is refused by [StartupTask.invoke] and recorded even if the runner catches it;
 * skipped tasks and recorded failures are checked once [run] returns. Either way the violation is a [StartupException] naming the components it applies
 ...
 * thread that waits for a lock this install cannot release. Declaring a dependency orders
 * its creation but does not make this lookup safe. Keep engine lookups on the installing
 * thread and arrange the data needed by worker tasks before dispatching their work.
 *
 ...
 * Android. The runner must honor thread affinity and the engine lookup restriction for
 * every initializer. For a [CoroutineInitializer], it also picks the thread `create` blocks
 * and whether that thread's pool can still make progress.
 */
