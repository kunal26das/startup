...
Dependency examples use **4.0.2**, which includes the task failure fixes described under
...
kotlin {
    sourceSets.commonMain.dependencies {
        api("io.github.kunal26das:startup:4.0.2")
    }
    listOf(iosArm64(), iosSimulatorArm64(), iosX64()).forEach { target ->
        target.binaries.framework {
            baseName = "Shared"
            export("io.github.kunal26das:startup:4.0.2")
        }
    }
}
```
